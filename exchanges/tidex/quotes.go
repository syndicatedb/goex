...
func (qp *QuotesProvider) Subscribe(symbol schemas.Symbol, d time.Duration, ch chan schemas.ResultChannel) {
...
// func (qp *QuotesProvider) SubscribeAll(d time.Duration, ch chan schemas.ResultChannel) {
// 	// ch := make(chan schemas.ResultChannel)
...
func (qp *QuotesProvider) SubscribeAll(callback func(string, string, interface{}, error)) {
	// ch := make(chan schemas.ResultChannel)

	for _, group := range qp.groups {
		go group.subscribe(callback)
		time.Sleep(100 * time.Millisecond)
	}
	// return ch
}
